...
# Approach 1: Calculate the sum of all the numbers in the list using the reduce function.
result = functools.reduce(lambda i, item: i + item, numbers)
print(result)

# Approach 2: Calculate the difference between the numbers in the list using the reduce function and a custom function.
def accum(i, item):
    print("counter => ", i)
    print("item => ", item)
    return i - item
